% create a list of tuple containing all the informations about each person
persons(0, []) :- !.
...
% if we don't have hint[(_|T) :- hint(T)] it won't display the list on the screen
...
% The question : Who owns the fish ? but we don't need to write this question fact and rule
% because we have already display all of the information in Persons in the array form.
